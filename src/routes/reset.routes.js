...
router.post('/company', resetPasswordLimiter, async (req, res) => {
  try {
    const { token, newPassword, email } = req.body;
    ...
    const [rows] = await db.query(
      'SELECT id, reset_token_expiry FROM companies WHERE reset_token = ? AND email = ?',
      [hashedToken, email]
    );
    ...
    if (new Date() > new Date(company.reset_token_expiry)) {
      return res.status(400).json({ error: 'Token has expired' });
    }
